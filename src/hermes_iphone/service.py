...
class IphoneService:
    def __init__(
        ...
        self.state = state or PluginState(
            root=(
                self.action_log_root.parent / "iphone-state"
                if action_log_root
                else None
            )
        )
    ...
    def _tree(self, udid: str | None = None) -> dict[str, Any]:
        ...
        return {
            "ok": True,
            "data": {"tree": compact_tree_from_xml(xml), "udid": udid},
    ...
    def _new_trace(
        ...
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S.%fZ")
        safe_action = (
            "".join(ch if ch.isalnum() or ch in "-_" else "-" for ch in action).strip(
                "-"
            )
            or "action"
        )
        ...
        if tree.get("ok"):
            (trace_dir / "tree-before.json").write_text(
                json.dumps(
                    tree.get("data", {}).get("tree", {}), indent=2, sort_keys=True
                )
            )
        try:
    ...
    def describe_screen(
        self, udid: str | None = None, limit: int = 20
    ) -> dict[str, Any]:
        ...
        summary = "\n".join(
            f"{el.get('id')} {el.get('type')}: {el.get('label')}" for el in elements
        )
        return {
            "ok": True,
            "data": {
    ...
    def last_trace(self) -> dict[str, Any]:
        traces = sorted(
            [p for p in self.trace_root.glob("*") if p.is_dir()], reverse=True
        )
        if not traces:
            return {
                "ok": False,
                "error": "trace_not_found",
                "message": "No iPhone trace folders found.",
            }
        latest = traces[0]
    ...
    def action_logs(self, limit: int = 10) -> dict[str, Any]:
        logs = sorted(self.action_log_root.glob("iphone-action-*.json"), reverse=True)[
            :limit
        ]
        entries = []
        for path in logs:
            try:
                entries.append(
                    {"path": str(path), "record": json.loads(path.read_text())}
                )
            except Exception:
    ...
    def find_element(
        ...
        matches = find_elements(
            tree, text=text, element_type=element_type, enabled=enabled, visible=visible
        )
        if not matches:
            return {
                "ok": False,
                "error": "element_not_found",
                "message": f"No visible element matched text={text!r} type={element_type!r}",
                "data": {"matches": [], "tree": tree},
            }
        return {
            "ok": True,
            "data": {"element": matches[0], "matches": matches, "udid": udid},
            "meta": {"backend": getattr(self.backend, "name", "unknown")},
        }

    def tap_element(
        self, element: dict[str, Any], udid: str | None = None
    ) -> dict[str, Any]:
        ...
        if result.get("ok"):
            ...
            result.setdefault("meta", {})["action_log"] = self._log_action(
                "tap_element", {"element": element, "udid": udid}
            )
            result.setdefault("meta", {})["trace_dir"] = trace["trace_dir"]
        return result

    def tap_element_id(
        self, element_id: str, udid: str | None = None
    ) -> dict[str, Any]:
        tree_result = self._tree(udid=udid)
        if not tree_result.get("ok"):
            return tree_result
        ...
        return {
            "ok": False,
            "error": "element_not_found",
            "message": f"No current element has id {element_id!r}",
            "data": {"element_id": element_id},
        }

    def tap_text(
        self, text: str, element_type: str | None = None, udid: str | None = None
    ) -> dict[str, Any]:
        found = self.find_element(
            text=text, element_type=element_type, enabled=True, udid=udid
        )
        if not found.get("ok"):
            return found
    ...
    def wait_for_text(
        ...
        return {
            ...
            "data": {"attempts": attempts, "last": last},
        }

    def type_into_field(
        self, field: str, text: str, udid: str | None = None
    ) -> dict[str, Any]:
        ...
        if typed.get("ok"):
            ...
            typed.setdefault("meta", {})["action_log"] = self._log_action(
                "type_into_field",
                {"field": element, "text_length": len(text), "udid": udid},
            )
        return typed
    ...
    def launch_or_focus(
        self, bundle_id: str, udid: str | None = None
    ) -> dict[str, Any]:
        current = self.current_app(udid=udid)
        if (
            current.get("ok")
            and (current.get("data") or {}).get("bundle_id") == bundle_id
        ):
            return {
                "ok": True,
                "data": {**current["data"], "already_foreground": True},
    ...
    def snapshot_state(
        self, udid: str | None = None, include_screenshot: bool = False, limit: int = 30
    ) -> dict[str, Any]:
        tree_result = self._tree(udid=udid)
        if not tree_result.get("ok"):
            return tree_result
        ...
        if include_screenshot:
            shot = self.screenshot(udid=udid)
            payload["screenshot"] = (
                (shot.get("data") or {}).get("path") if shot.get("ok") else None
            )
            if not shot.get("ok"):
    ...
    def select_suggestion(self, query: str, udid: str | None = None) -> dict[str, Any]:
        ...
        if not candidates:
            return {
                ...
                "message": f"No visible suggestion matched {query!r}",
                "data": {"query": query},
            }
        ...
        tapped = self.tap_element(element, udid=udid)
        if tapped.get("ok"):
            tapped.setdefault("data", {})["suggestion"] = {
                "label": label,
                "element": element,
            }
        return tapped
    ...
    def dismiss_keyboard(self, udid: str | None = None) -> dict[str, Any]:
        ...
        return {
            "ok": False,
            "error": "keyboard_dismiss_control_not_found",
            "message": "No visible Done/Hide keyboard/Return control was found.",
            "data": {"udid": udid},
        }
    ...
    def go_back(self, udid: str | None = None) -> dict[str, Any]:
        for text in ("Back", "Messages", "Cancel", "Close"):
            found = self.find_element(
                text=text, element_type="button", enabled=True, udid=udid
            )
            if found.get("ok"):
        ...
        return {
            "ok": False,
            "error": "back_control_not_found",
            "message": "No visible Back/Messages/Cancel/Close button was found.",
            "data": {"udid": udid},
        }

    def recover_to_home_or_app(
        self, bundle_id: str | None = None, udid: str | None = None
    ) -> dict[str, Any]:
        ...
        if not bundle_id:
            return {
                "ok": True,
                "data": {"recovered_to": "home", "udid": udid},
                "meta": home.get("meta", {}),
            }
        launched = self.launch_app(bundle_id=bundle_id, udid=udid)
        if launched.get("ok"):
    ...
    def prepare_text(
        self, to: str, body: str, udid: str | None = None
    ) -> dict[str, Any]:
        ...
        if (
            not selected_recipient.get("ok")
            and selected_recipient.get("error") != "recipient_suggestion_not_found"
        ):
        ...
        screenshot_path = (
            (screenshot.get("data") or {}).get("path") if screenshot.get("ok") else None
        )
        send = self.find_element(
            text="Send", element_type="button", enabled=True, udid=udid
        )
        if not send.get("ok"):
            return send
        ...
        payload = {
            ...
            "screenshot_before_send": screenshot_path,
            "recipient": recipient,
        }
    ...
    def _select_text_recipient_suggestion(
        self, to: str, udid: str | None = None
    ) -> dict[str, Any]:
        tree_result = self._tree(udid=udid)
        if not tree_result.get("ok"):
            return tree_result
        ...
        for element in elements:
            ...
            if (
                element.get("type") in {"cell", "button"}
                and query
                and query in normalized
                and ("maybe:" in normalized or normalized != query)
            ):
                tapped = self.tap_element(element, udid=udid)
                if not tapped.get("ok"):
                    return tapped
                ...
                return {
                    "ok": True,
                    "data": {
                        "verified_by": "contact_suggestion",
                        "display": display,
                        "element": element,
                    },
                }
        return {
            "ok": False,
            "error": "recipient_suggestion_not_found",
            "message": f"No Messages contact suggestion matched {to!r}",
        }

    def _verify_text_recipient(
        self, to: str, udid: str | None = None
    ) -> dict[str, Any]:
        tree_result = self._tree(udid=udid)
        if not tree_result.get("ok"):
        ...
        for element in elements:
            label = (
                str(element.get("label") or element.get("name") or "").strip().lower()
            )
            ...
            if element.get("type") in {
                ...
                if not value or query in value:
                    verified_by = (
                        "contact_suggestion" if value and value != query else "to_field"
                    )
                    return {
                        "ok": True,
                        "data": {
    ...
    def open_messages_thread(
        self, contact: str, udid: str | None = None
    ) -> dict[str, Any]:
        launched = self.launch_or_focus("com.apple.MobileSMS", udid=udid)
        if not launched.get("ok"):
            return launched
        visible = self.find_element(
            text=contact, element_type="cell", enabled=True, udid=udid
        )
        if visible.get("ok"):
        ...
        if not search.get("ok"):
            return {
                "ok": False,
                "error": "messages_thread_not_found",
                "message": f"No visible Messages thread or Search field matched {contact!r}.",
                "data": {"contact": contact, "launch": launched},
            }
        ...
        return {
            "ok": False,
            "error": "messages_thread_not_found",
            "message": f"Could not open Messages thread for {contact!r} after searching.",
            "data": {"contact": contact, "last": result},
        }

    def read_recent_messages(
        self, limit: int = 10, udid: str | None = None
    ) -> dict[str, Any]:
        tree_result = self._tree(udid=udid)
        if not tree_result.get("ok"):
            return tree_result
        ...
        if tree.get("bundle_id") != "com.apple.MobileSMS":
            return {
                "ok": False,
                "error": "messages_not_foreground",
                "message": "Messages is not the foreground app.",
                "data": {"bundle_id": tree.get("bundle_id"), "name": tree.get("name")},
            }
        ignored = {
            "messages",
            "compose",
        ...
        for element in elements:
            if element.get("type") == "application":
                try:
                    app_width = int(
                        (element.get("bounds") or {}).get("width") or app_width
                    )
                except Exception:
                    pass
                break
        ...
        if app_width >= 700:
            for element in elements:
                if element.get("name") == "messageBodyField":
                    try:
                        pane_left = max(
                            0, int((element.get("bounds") or {}).get("x", 0)) - 80
                        )
                    except Exception:
                        pane_left = 0
                    break
        ...
        for element in elements:
            ...
            label = str(
                (
                    element.get("value")
                    if element_type == "textview" and element.get("value")
                    else None
                )
            ...
            if not label or normalized in ignored:
                continue
            if normalized.startswith("read ") or re.fullmatch(
                r"\d{1,2}:\d{2}\s*(am|pm)?", normalized
            ):
                continue
    ...
    def prepare_current_message_reply(
        self, body: str, udid: str | None = None
    ) -> dict[str, Any]:
        current = self.current_app(udid=udid)
        ...
        if (current.get("data") or {}).get("bundle_id") != "com.apple.MobileSMS":
            return {
                "ok": False,
                "error": "messages_not_foreground",
                "message": "Open a Messages thread before preparing a reply.",
        ...
        payload = {
            "to": "current Messages thread",
            "body_length": len(body),
            "udid": udid,
            "send_element": element,
            "screenshot_before_send": screenshot_path,
            "thread_context": (context.get("data") or {}),
        }
        prepared = self.policy.prepare("send_text", payload)
        self._log_action(
            "prepare_current_message_reply",
            {
                "to": payload["to"],
                "body_length": len(body),
                "udid": udid,
    ...
    def send_text(
        ...
        if approval not in {"once", "session", "always"}:
            ...
            message = (
                ...
                else "Hermes approval denied the staged send; the draft remains unsent."
            )
            return {
                "ok": False,
                "error": error,
    ...
    def confirm_prepared_action(
        self, token: str, udid: str | None = None
    ) -> dict[str, Any]:
        ...
        result = (
            self.tap_element(element, udid=actual_udid)
            if isinstance(element, dict)
            else self.tap_text("Send", element_type="button", udid=actual_udid)
        )
        if result.get("ok"):
            self._log_action(
                "confirm_send_text",
            ...
            return {
                ...
                "data": {
                    ...
                    "to": payload.get("to"),
                    "body_length": payload.get("body_length"),
                    "screenshot_before_send": payload.get("screenshot_before_send"),
                },
