...
def register(ctx) -> None:
    ...
    _register(ctx, "iphone_snapshot_state", "Return current app plus compact semantic UI state; optionally include a screenshot path.", {"include_screenshot": {"type": "boolean", "default": False}, "limit": {"type": "integer", "default": 30}, "udid": UDID}, [], lambda a: _service().snapshot_state(include_screenshot=bool(a.get("include_screenshot", False)), limit=int(a.get("limit", 30)), udid=a.get("udid")))
    _register(ctx, "iphone_select_suggestion", "Tap the best visible suggestion/cell/button matching query text.", {"query": {"type": "string"}, "udid": UDID}, ["query"], lambda a: _service().select_suggestion(query=a["query"], udid=a.get("udid")))
    _register(ctx, "iphone_dismiss_keyboard", "Dismiss the visible software keyboard when a Done/Hide keyboard/Return control is available.", {"udid": UDID}, [], lambda a: _service().dismiss_keyboard(udid=a.get("udid")))
    _register(ctx, "iphone_go_back", "Tap a visible Back/Messages/Cancel/Close navigation control.", {"udid": UDID}, [], lambda a: _service().go_back(udid=a.get("udid")))
    _register(ctx, "iphone_recover_to_home_or_app", "Press Home, then optionally launch a target bundle id to recover to a known state.", {"bundle_id": {"type": "string"}, "udid": UDID}, [], lambda a: _service().recover_to_home_or_app(bundle_id=a.get("bundle_id"), udid=a.get("udid")))
    ...
    _register(ctx, "iphone_open_messages_thread", "Open a Messages conversation by visible thread cell or Messages search.", {"contact": {"type": "string"}, "udid": UDID}, ["contact"], lambda a: _service().open_messages_thread(contact=a["contact"], udid=a.get("udid")))
    _register(ctx, "iphone_read_recent_messages", "Read the last N visible message-like items from the current Messages screen.", {"limit": {"type": "integer", "default": 10}, "udid": UDID}, [], lambda a: _service().read_recent_messages(limit=int(a.get("limit", 10)), udid=a.get("udid")))
    _register(ctx, "iphone_prepare_current_message_reply", "Draft a reply in the currently open Messages thread and stage Send behind a confirmation token. This never taps Send.", {"body": {"type": "string"}, "udid": UDID}, ["body"], lambda a: _service().prepare_current_message_reply(body=a["body"], udid=a.get("udid")))
    _register(ctx, "iphone_reply_current_message_thread", "Reply in the currently open Messages thread after Hermes built-in approval. Stages and verifies the draft before tapping Send.", {"body": {"type": "string"}, "udid": UDID}, ["body"], lambda a: _service().reply_current_message_thread(body=a["body"], udid=a.get("udid")))
