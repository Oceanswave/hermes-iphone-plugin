...
def test_ensure_wda_prefers_native_lifecycle_before_shell_helper(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda udid=None: (
            (False, None, "down")
            if len(calls) == 0
            else (True, {"value": {"ready": True}}, None)
        ),
    )
    monkeypatch.setattr(
        backend,
        "_ensure_wda_native",
        lambda udid=None: (
            calls.append(("native", udid))
            or BackendResult(ok=True, data={"started_wda": True, "method": "native"})
        ),
    )
    monkeypatch.setattr(
        backend,
        "_ensure_wda_helper",
        lambda udid=None: calls.append(("helper", udid)) or BackendResult(ok=True),
    )
...
def test_ensure_wda_falls_back_to_helper_when_native_lifecycle_fails(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda udid=None: (
            readiness["ready"],
            {"value": {"ready": True}} if readiness["ready"] else None,
            None if readiness["ready"] else "down",
        ),
    )
    monkeypatch.setattr(
        backend,
        ...
        lambda udid=None: (
            calls.append(("native", udid))
            or BackendResult(ok=False, error="native_failed")
        ),
    )
